          ...
          if(item.nestedItem.length > 0){
            return (
              <section className='flex flex-row justify-between items-center' key={item.name}>
                <NavLink
                  to={item.path}
                  className={`w-full flex items-center py-2.5 px-4 rounded-2xl transition duration-200 ${
                    active === item.name ? 'bg-black text-slate-50' : 'hover:bg-black  hover:text-slate-50'
                  }`}
                ...
                {item.nestedItem.length > 0 && (<BiUpArrow className="ml-auto" size={8} onClick={()=>setShowDropdown(true)}/>)}
